...
class Operators:
    ...
    # Change define to make it work with new dictstack format
    ...
    """
    # Implement for both static and dynamic lookup
    def lookup(self,name):
        name = '/' + name
        if self.scope == 'static':
            return self.staticLookup(name, len(self.dictstack) - 1)
        else:
    ...
    # Algorithm taken from lab 3
    def staticLookup(self, name, i):
        if name in self.dictstack[i][1]:
            return self.dictstack[i][1][name]
        elif i == 0:
            return None
        else:
            return self.staticLookup(name, self.dictstack[i][0])
    ...
    def stack(self):
        ...
        print("===**dictstack***===")
        if len(self.dictstack) > 0:
            stack = list(reversed(self.dictstack))
            for i in range(0, len(stack)):
                print(f"--- {len(stack) - 1 - i} ---- {stack[i][0]} ----")
                for k, v in stack[i][1].items():
                    print(f"{k}   {v}")
        else:
    ...
    def psIf(self):
        ...
        if condition and isinstance(ifbody, FunctionValue):
            # Pushing new tuple (AR, {}) before apply
