    ...
    def evaluate(self,psstacks):
        if self.var_name[0] == '/':
            ...
            if isinstance(val, FunctionValue):
                ...
                psstacks.dictPush((self.getStaticLink(psstacks, self.var_name), {}))
                val.apply(psstacks)
            else:
                psstacks.opPush(val)
    
    # Helper function to get static link values
    def getStaticLink(self, psstacks, name):
        name = '/' + name
        for i in range (len(psstacks.dictstack)):
            if name in psstacks.dictstack[i][1]:
                return i
